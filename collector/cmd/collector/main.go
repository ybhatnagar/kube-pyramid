...
import (
	...
	"github.com/kube-pyramid/collector/internal/config"
	"github.com/kube-pyramid/collector/internal/connectors"
	_ "github.com/kube-pyramid/collector/internal/connectors/prometheus" // self-registers "prometheus"
	"github.com/kube-pyramid/collector/internal/ingest"
	"github.com/kube-pyramid/collector/internal/server"
...
func usage() {
	fmt.Fprint(os.Stderr, `collector — Kube Pyramid data collector

Usage:
  collector ingest [--all|--metrics|--allocations|--interactions] [flags]  ingest into the state DB
...
Common flags:
  --db-driver   sqlite|postgres            (env KUBEPYRAMID_DB_DRIVER, default sqlite)
  --db-dsn      connection string / path   (env KUBEPYRAMID_DB_DSN, default ./kubepyramid.db)
  --config      path to a JSON config file (flags > env > file > default)

Ingest flags:
  --source            metrics/allocations connector  (default prometheus)
  --interaction-source hubble|istio|otel             (env KUBEPYRAMID_INTERACTION_SOURCE, default hubble)
  --prom-url    Prometheus endpoint        (env KUBEPYRAMID_PROM_URL, default http://prometheus.monitoring:9090)
  --prom-bearer bearer token for Prometheus (env KUBEPYRAMID_PROM_BEARER)
...
func openStore(r *config.Resolver) (store.StateStore, error) {
	driver := r.String("db-driver", "KUBEPYRAMID_DB_DRIVER", "sqlite")
	def := "./kubepyramid.db"
	if driver != "sqlite" && driver != "" {
		def = "" // no safe default DSN for postgres
	}
	dsn := r.String("db-dsn", "KUBEPYRAMID_DB_DSN", def)
	if dsn == "" {
...
func cmdIngest(args []string) error {
	...
	doAllocations := fs.Bool("allocations", false, "run the allocations step")
	...
	selected := selectedSteps(*all, *doMetrics, *doAllocations, *doInteractions)
	sourceName := r.String("source", "", "prometheus")
	interactionSource := r.String("interaction-source", "KUBEPYRAMID_INTERACTION_SOURCE", "hubble")
	...
	req := ingest.Request{
		...
		Endpoint:          r.String("prom-url", "KUBEPYRAMID_PROM_URL", "http://prometheus.monitoring:9090"),
		Auth:              connectors.AuthConfig{Bearer: r.String("prom-bearer", "KUBEPYRAMID_PROM_BEARER", "")},
...
func cmdConnectors(args []string) error {
	...
	metrics, allocations, interactions := connectors.Names()
	...
	printList("allocations connectors", allocations)
...
func cmdServe(args []string) error {
	...
	deps := server.Deps{
		...
		DefaultInteractionSource: r.String("interaction-source", "KUBEPYRAMID_INTERACTION_SOURCE", "hubble"),
		DefaultEndpoint:          r.String("prom-url", "KUBEPYRAMID_PROM_URL", "http://prometheus.monitoring:9090"),
		DefaultAuth:              connectors.AuthConfig{Bearer: r.String("prom-bearer", "KUBEPYRAMID_PROM_BEARER", "")},
...
func selectedSteps(all, metrics, allocations, interactions bool) []string {
	if all {
		return []string{"metrics", "allocations", "interactions"}
	}
	var out []string
	if metrics {
