...
// UpsertAllocations writes the N-dimensional allocation vector (tier 3, allocations).
func (s *SQLStore) UpsertAllocations(ctx context.Context, rows []AllocationRow) (int, error) {
	if len(rows) == 0 {
		return 0, nil
	}
	q := s.rebind(`INSERT INTO allocations
	...
	err := s.inTx(ctx, func(tx *sql.Tx) error {
		...
		for _, a := range rows {
			if _, err := stmt.ExecContext(ctx,
				a.ClusterID, a.WorkloadUID, a.Resource, resourceKindOrDefault(a.ResourceKind),
				nullF64(a.Requested), nullF64(a.Limit), nullStr(a.Unit),
				s.boolArg(a.IsCustom), nullStr(a.Source), s.ts(a.CollectedAt),
			); err != nil {
				return err
			}
		}
