...
func init() {
	...
	connectors.RegisterAllocations(c) // KSM requests/limits -> allocation vector
}
...
// --- allocations (KSM requests/limits) -------------------------------------

// defaultAllocQueries returns the requests/limits PromQL, joined pod->workload via the
// kubernetes-mixin owner recording rule and grouped by resource. kube-state-metrics'
// `resource` label already carries extended/custom resources (e.g. nvidia_com_gpu), so
// this one PromQL source covers the full N-dimensional vector. Both are overridable via
// Config.Extra["alloc_requests_query"] / ["alloc_limits_query"].
func defaultAllocQueries() (requests, limits string) {
	join := func(metric string) string {
		return `sum by (namespace, workload, workload_type, resource, unit) (` +
			metric + `{namespace=~"$NS"}` +
			` * on (namespace, pod) group_left (workload, workload_type) ` +
			`namespace_workload_pod:kube_pod_owner:relabel{namespace=~"$NS"})`
	}
	return join("kube_pod_container_resource_requests"), join("kube_pod_container_resource_limits")
}

// FetchAllocations reads requests + limits from kube-state-metrics and merges them into
// one AllocationRow per (workload, resource). Allocations are ~constant, so the last
// value in the window is used.
func (c *Connector) FetchAllocations(ctx context.Context, w connectors.Window, cfg connectors.Config) ([]store.AllocationRow, error) {
	nsRe := ".+"
	if len(cfg.Namespaces) > 0 {
		nsRe = strings.Join(cfg.Namespaces, "|")
	}
	reqQ, limQ := defaultAllocQueries()
	if v := cfg.Extra["alloc_requests_query"]; v != "" {
		reqQ = v
	}
	if v := cfg.Extra["alloc_limits_query"]; v != "" {
		limQ = v
	}
	collectedAt := time.Now().UTC()

	rows := map[string]*store.AllocationRow{} // key = uid|resource
	key := func(uid, res string) string { return uid + "|" + res }

	apply := func(query string, set func(a *store.AllocationRow, v float64)) error {
		series, err := c.queryRange(ctx, cfg, strings.ReplaceAll(query, "$NS", nsRe), w)
		if err != nil {
			return err
		}
		for _, s := range series {
			uid, _, _, _, ok := workloadFromLabels(s.Metric)
			res := s.Metric["resource"]
			if !ok || res == "" || len(s.Values) == 0 {
				continue
			}
			a, exists := rows[key(uid, res)]
			if !exists {
				a = &store.AllocationRow{
					ClusterID: cfg.ClusterID, WorkloadUID: uid, Resource: res,
					ResourceKind: store.ResourceKindFor(res), Unit: s.Metric["unit"],
					IsCustom: store.IsCustomResource(res), Source: "ksm", CollectedAt: collectedAt,
				}
				rows[key(uid, res)] = a
			}
			set(a, s.Values[len(s.Values)-1].v) // last value in window
		}
		return nil
	}

	if err := apply(reqQ, func(a *store.AllocationRow, v float64) { a.Requested = ptrF(v) }); err != nil {
		return nil, fmt.Errorf("requests query: %w", err)
	}
	if err := apply(limQ, func(a *store.AllocationRow, v float64) { a.Limit = ptrF(v) }); err != nil {
		return nil, fmt.Errorf("limits query: %w", err)
	}

	out := make([]store.AllocationRow, 0, len(rows))
	for _, a := range rows {
		out = append(out, *a)
	}
	sort.Slice(out, func(i, j int) bool {
		if out[i].WorkloadUID != out[j].WorkloadUID {
			return out[i].WorkloadUID < out[j].WorkloadUID
		}
